...
function checkButtonUpdate() {
  checkButton.disabled = countSavedAnswers() != true;
};
...
function correctAnswers() {
  ...
  modalContainer1.setAttribute('class', 'modal');
  modalContainer1.style.visibility = 'visible';
  modalContainer1.style.opacity = '1';

  const modalContainer2 = document.createElement('div');
  modalContainer2.setAttribute('class', 'modal-content');

  const modalSpan = document.createElement('span');
  modalSpan.setAttribute('class', 'modal-close')
  modalSpan.textContent = "x"; // Agrega un símbolo de cierre
  modalSpan.addEventListener('click', () => {
    modalContainer1.remove(); // Elimina el modal del DOM
  });

  const modalH2 = document.createElement('h2');
  modalH2.textContent = "Results";

  const modalP = document.createElement('p');
  modalP.textContent = count + " of " + pageObj.length;


  modalContainer1.appendChild(modalContainer2);
  modalContainer2.appendChild(modalSpan);
  modalContainer2.appendChild(modalH2);
  modalContainer2.appendChild(modalP);
